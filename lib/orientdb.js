...
class Database {
    ...
    const LoadModels = (path) => {
      ...
      files.forEach((file) => {
        if (file.isDirectory()) {
          ...
          const model = JSON.parse(fs.readFileSync(`${pathname}/${file.name}`, 'utf8'));
          const { name } = model;
  ...
  async UpdateItem(model, attrs, id) {
    ...
    const result = await this.updateQuery(Model, id);
    if (!result || result.count === undefined) return -1;
    return result.count;
  }
  ...
  async DeleteItem(model, id) {
    ...
    const result = await this.deleteQuery(Model, id);
    if (!result || result.count === undefined) return -1;
    return result.count;
  }
  ...
  async simpleCount(where, id) {
    const whereOption = where && where.length ? `WHERE ${where}` : '';
    try {
  ...
  updateQuery(_model, where) {
    ...
    if (attrs.created_at !== undefined) delete attrs.created_at;
    if (!Object.keys(attrs).length) return 0;
    ...
    let whereQuery;
    if (typeof where === 'string') whereQuery = `WHERE ${where}`;
    else if (typeof where === 'number') whereQuery = `WHERE id=${where}`;
    else return null;

    const query = `UPDATE ${id} SET ${SetQuery} ${whereQuery}`;
  ...
  deleteQuery(_model, where) {
    ...
    if (!model) return null;
    while (model && model !== 'V' && model !== 'E') {
      if (model.extend === 'V' || model.extend === 'E') extend = model.extend;
      model = this.models[model.extend] || null;
    }
    ...
    if (extend === 'V') commandQuery = `DELETE VERTEX ${id}`;
    else if (extend === 'E') return 0;
    else commandQuery = `DELETE FROM ${id}`;

    const query = `${commandQuery} ${whereQuery}`;
    return this.command(query);
  }
